module.exports = {
	...
	execute: async (bot, msg, args) => {
		...
		resp = await msg.channel.awaitMessages({filter: m => m.author.id == msg.author.id, time:1000*60, max:1});
		if(!resp.first()) return "Action cancelled: timed out.";
		if(!(category && resp.first().content.toLowerCase() == "skip"))
			category = await msg.guild.channels.cache.find(c => [c.id,c.name.toLowerCase()].includes(resp.first().content) && c.type == 'GUILD_CATEGORY');
		...
		resp = await msg.channel.awaitMessages({filter: m => m.author.id == msg.author.id, time:1000*60, max:1});
		if(!resp.first()) return "Action cancelled: timed out.";
		if(resp.first().content.toLowerCase() != "skip")
			archives = msg.guild.channels.cache.find(c => [c.id,c.name.toLowerCase()].includes(resp.first().content.replace(/[<#>]/g, "")) && c.type == 'GUILD_TEXT');
