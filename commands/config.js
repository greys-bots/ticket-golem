...
module.exports.subcommands.category = {
	...
	execute: async (bot, msg, args) => {
		...
		var scc;
		if(!cfg) {
			scc = await bot.utils.createConfig(bot, msg.guild.id, category.id, null);
		} else {
			scc = await bot.utils.updateConfig(bot, msg.guild.id, "category_id", category.id);
		}
...
module.exports.subcommands.archive = {
	...
	execute: async (bot, msg, args) => {
		...
		var scc;
		if(!cfg) {
			scc = await bot.utils.createConfig(bot, msg.guild.id, null, archives.id);
		} else {
			scc = await bot.utils.updateConfig(bot, msg.guild.id, "archives_id", archives.id);
		}
